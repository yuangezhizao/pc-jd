...
pageSize=10& \ #每页10条评论
isShadowSku=0
https://club.jd.com/comment/productPageComments.action?callback=fetchJSON_comment98vv3409&productId=4483104&score=0&sortType=5&page=919&pageSize=10&isShadowSku=0
备用url: 
...
def get_sku_group():
    ...
    sql = 'select sku_group from sku_phone_jd \
           where shop_id="1000000127" and sku_name like "%iphone 7%" '
    sku_group_list = pd.read_sql(sql, conn)
    conn.close()
    if len(sku_group_list) == 0:
        print('message:没有待抓取任务')
    return sku_group_list

#获取sku_group评论的页数，若失败，返回0
def get_pages(sku_group):
    url = 'https://club.jd.com/comment/productPageComments.action'
    params = {'callback':'fetchJSON_comment98vv3409',
              'productId':sku_group,
              'score':0,
              'sortType':6,
              'page':0,
    ...
    try:
        ...
        comment_dict = json.loads(txt[26:-2]) #截取长度和fetchJSON_comment98vv116有关
...
def parse(txt):
    ...
    comment_dict = json.loads(txt[26:-2])
    image_list_count = comment_dict.get('imageListCount', 0)
    comments = comment_dict['comments']
    comments_list = []
    if len(comments) > 0 and comments != None:
...
def input_mysql(record):
    ...
    cur = conn.cursor()
    sql = 'insert into comment_phone_jd(category_level3_id, brand_id, sku_group, max_page,\
...
def main():
    ...
    brand_id = '18374'                                           #1.类别-品牌
    sku_group_list = pd.DataFrame({'sku_group':['4099139']})    #2.sku列表
